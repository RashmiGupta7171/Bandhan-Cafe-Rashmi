...
// // ADD THIS FUNCTION inside your Finance component

// const editRecord = async (rec) => {
//   const newRevenue = prompt("Edit Revenue", rec.revenue);
//   const newExpenses = prompt("Edit Expenses", rec.expenses);

//   if (!newRevenue || !newExpenses) return;

//   try {
//     await fetch(`http://localhost:5000/api/finance/${rec._id}`, {
//       method: "PUT",
//       headers: {
//         "Content-Type": "application/json",
//       },
...
//                 </td>
//                 <td style={styles.td}>
//                     <button onClick={() => editRecord(rec)}>✏️</button>
...
function Finance() {
  ...
  const editRecord = async (rec) => {
    ...
    if (newRevenue === null || newExpenses === null) return;
          ...
          {records.length === 0 ? (
            ...
            records.map((rec, i) => (
              <tr key={rec._id}>
                <td style={styles.td}>{i + 1}</td>
                <td style={styles.td}>{rec.date}</td>
                <td style={styles.td}>₹{rec.revenue}</td>
                <td style={styles.td}>₹{rec.expenses}</td>
                <td style={{ ...styles.td, color: "green", fontWeight: "bold" }}>
                  ₹{rec.profit}
                </td>
                <td style={styles.td}>
